...
def computeJoints(namae):
    ...
    cv2.imwrite(checking_path + namae + '_check.'+data_type, img)
    # cv2.imwrite(checking_path + namae + '_checkOri.png', oriImg)

    # ============ save joints coordinates to csv array
    tempArr=[]
    for fjnt in joints:
        for jnt in fjnt:
            tempArr.append(jnt)
    tempArr.insert(0, int(namae))
    _jntcsv.append(np.hstack(tempArr))
